...
- **自动安装便携版 Node.js**（免管理员权限；便携模式——不写注册表、不改系统 PATH，全部装在包目录内）
- **自动安装 dsh**：本地 `npm install --prefix` 装进包目录（无全局安装、无系统改动；重试时自动切换国内镜像）
...
- 检查 Node.js v22+：缺失或过旧时，从 npmmirror（备用：nodejs.org）下载最新 LTS `win-x64`
  便携版，解压到启动器旁 —— 便携模式：不写注册表、不改系统 PATH（会话 PATH 由本地标记文件重建）
...
zip 内的 `使用说明.txt`（中文）和 `README-EN.txt`（英文）有完整 FAQ：SmartScreen 弹窗、微信传文件损坏、目录不可写、
